...
def render(viewpoint_camera: Camera, pc: GaussianModel, pipe: PipelineParams, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, stage="fine", extra_cams: "list[Camera]" = None):
    ...
    if stage == "coarse":
        ...
        means3D_deform, scales_deform, rotations_deform, opacity_deform, velocity, confidence = pc._deformation(
    ...
    rendered_image, radii, depth, rendered_alpha, proj_means_2D, conic_2D, conic_2D_inv, gs_per_pixel, weight_per_gs_pixel, x_mu = rasterizer(
        means3D = means3D_final,
        means2D = means2D,
        shs = shs,
        colors_precomp = colors_precomp,

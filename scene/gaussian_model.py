...
class GaussianModel:
    ...
    def restore(self, model_args, training_args):
        ...
        self._deformation.load_state_dict(_deformation_dict, strict=False)
    ...
    def load_model(self, path):
        print(f"loading model from exists {path}")
        weight_dict = torch.load(os.path.join(path,"deformation.pth"),map_location="cuda")
        velocity_names = ["deformation_net.velocity_head.1.weight", "deformation_net.velocity_head.1.bias", "deformation_net.velocity_head.3.weight", "deformation_net.velocity_head.3.bias"]
        if velocity_names[0] not in self._deformation.state_dict() and velocity_names[0] in weight_dict:
            print("Removing velocity head from checkpoint")
            for k in velocity_names:
                del weight_dict[k]
        confidence_names = ["deformation_net.confidence_head.1.weight", "deformation_net.confidence_head.1.bias", "deformation_net.confidence_head.3.weight", "deformation_net.confidence_head.3.bias"]
        if confidence_names[0] not in self._deformation.state_dict() and confidence_names[0] in weight_dict:
            print("Removing confidence head from checkpoint")
            for k in confidence_names:
                del weight_dict[k]
